...
export const GET = async (request: Request, { params }: { params: { id: string } }) => {
    try {
      ...
      const song = await Song.findOne({ spotify_id: params.id }).exec();
      if (!song) {
  ...
  try {
    ...
    if (!song_id || !user_id || !text || rating == null) {
      return NextResponse.json(
        { error: "Missing required fields: songId, userId, text, or rating" },
        { status: 400 }
      );
    }
    ...
    if (text.length > 250) {
      return NextResponse.json(
        { error: "Review text cannot exceed 250 characters" },
        { status: 400 }
      );
    }
